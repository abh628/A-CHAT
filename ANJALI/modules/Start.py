...
import psutil
import config
from ANJALI import _boot_
from ANJALI import get_readable_time
from ANJALI import ANJALI, mongo
...
from ANJALI.database.chats import get_served_chats, add_served_chat
from ANJALI.database.users import get_served_users, add_served_user
from pyrogram.types import InlineKeyboardButton, InlineKeyboardMarkup, Message, CallbackQuery
from ANJALI.modules.helpers import (
    START,
    START_BOT,
    PNG_BTN,
    CLOSE_BTN,
...
@ANJALI.on_message(filters.new_chat_members)
        ...
        for member in message.new_chat_members:
            ...
            if member.id == ANJALI.id:
                try:
                    ...
                    pass
                try:
                    invitelink = await ANJALI.export_chat_invite_link(message.chat.id)
                ...
                try:
                    groups_photo = await ANJALI.download_media(
                ...
                count = await ANJALI.get_chat_members_count(chat.id)
                chats = len(await get_served_chats())
...
async def start(_, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        if m.chat.photo:
            try:
                userss_photo = await ANJALI.download_media(m.chat.photo.big_file_id)
                await umm.delete()
                if userss_photo:
        ...
        await m.reply_photo(photo=chat_photo, caption=START.format(ANJALI.mention or "can't mention", users, chats, UP), reply_markup=InlineKeyboardMarkup(START_BOT))
        await add_served_user(m.chat.id)
        keyboard = InlineKeyboardMarkup([[InlineKeyboardButton(f"{m.chat.first_name}", user_id=m.chat.id)]])
        await ANJALI.send_photo(int(OWNER_ID), photo=chat_photo, caption=f"{m.from_user.mention} ʜᴀs sᴛᴀʀᴛᴇᴅ ʙᴏᴛ. \n\n**ɴᴀᴍᴇ :** {m.chat.first_name}\n**ᴜsᴇʀɴᴀᴍᴇ :** @{m.chat.username}\n**ɪᴅ :** {m.chat.id}\n\n**ᴛᴏᴛᴀʟ ᴜsᴇʀs :** {users}", reply_markup=keyboard)
                ...
                for chat in chats:
                    ...
                    try:
                        if broadcast_type == "reply":
                            m = await ANJALI.forward_messages(
                                chat_id, message.chat.id, [broadcast_content.id]
                            )
                        else:
                            m = await ANJALI.send_message(
